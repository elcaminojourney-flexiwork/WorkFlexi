...
export default function TimesheetInvoiceDetails() {
  ...
  const loadInvoiceDetails = async (id: string) => {
    try {
      ...
      // Load invoice data directly from timesheet (as per spec - no separate invoices table)
      console.log('[INVOICE PAGE] Loading invoice from timesheet ID:', id);
      ...
      if (tsError || !timesheetData) {
        throw new Error('Timesheet not found');
      }

      const ts = timesheetData as TimesheetDetails;
      setTimesheet(ts);
      ...
      // 3) Load employer profile
      const { data: employerData, error: employerError } = await supabase
        .from('profiles')
        .select('id, company_name, email, phone, location')
        .eq('id', user.id)
        .single();
      ...
      // 4) Load worker profile
      if (ts.worker_id) {
        const { data: workerData, error: workerError } = await supabase
          .from('profiles')
          .select('id, full_name, phone, bank_account_number')
          .eq('id', ts.worker_id)
          .single();
      ...
      // 5) Calculate payment breakdown using same logic as payment summary
      const hoursCalculation = calculateHoursFromTimes({
        clockIn: ts.clock_in_time,
        clockOut: ts.clock_out_time,
        breakMinutes: ts.break_duration_minutes ?? undefined,
        minimumHours: 4,
      ...
      const invoiceNum = generateInvoiceNumber(ts.id, ts.shift_id);
      setInvoiceNumber(invoiceNum);
    } catch (err: any) {
      console.error('Error loading invoice details:', err);
      Alert.alert('Error', err?.message || 'Unable to load invoice details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateInvoiceNumber = (id: string, shiftId?: string) => {
    // Generate invoice number from shift ID and date, or use provided ID
    ...
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.invoiceHeader}>
          <View>
            ...
            <Text style={styles.invoiceNumber}>
              {invoiceNumber || generateInvoiceNumber(timesheet?.id || shift.id, shift.id)}
            </Text>
          </View>
          ...
          <Card mode="elevated" style={{ marginBottom: 12 }}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValueBold}>
                {paymentData.regularHours.toFixed(2)} hours
              </Text>
            </View>
            <View style={styles.detailRow}>
        ...
        <Card mode="elevated" style={{ marginBottom: 12 }}>
          ...
          <View style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>Subtotal (Worker Cost):</Text>
            <Text style={styles.breakdownValue}>
          ...
          <View style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>
              Platform Fee ({((paymentData.platformFeePercentage || 0) * 100).toFixed(0)}%):
            </Text>
            <Text style={styles.breakdownValue}>
              {formatCurrency(paymentData.platformFee)}
            </Text>
          </View>
          ...
          <View style={styles.totalRow}>
            <Text style={styles.totalLabel}>TOTAL CHARGED (To Employer):</Text>
            <Text style={styles.totalValue}>
              {formatCurrency(paymentData.totalCharged)}
            </Text>
          </View>
